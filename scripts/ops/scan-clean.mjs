...
// wordlist 完整性清单是 sha256 文件摘要(by-design, 防篡改校验), 其 64 位 hex 非令牌 — 仅豁免 hex 规则, 其余规则照扫
const DIGEST_OK = /assets[/\\]wordlists[/\\]manifest\.json$|wordlists\.test\.mjs$/
  ...
  for (const ent of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    if (ent.isDirectory()) walk(p)
      ...
      for (const [label, re] of PATTERNS) {
        if (label === '64位hex疑似令牌' && DIGEST_OK.test(p)) continue
