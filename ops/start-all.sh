...
#
# 0913 加固(审查采纳):
#   - PID 文件管理(${D2D_DATA_DIR}/run/*.pid): stop 子命令优雅终止, 不再依赖全库 pkill
#   - graphd 启动后 /health 健康检查(失败 fast-fail, 不再盲等 2s 后带病启动全家)
#   - 兜底 pkill 锚定本仓库绝对路径, 不误杀其他项目的同名脚本/其他 dsh 实例
...
# 优雅终止: PID 文件优先(TERM→等 5s→KILL), 兜底 pkill 只锚定本仓库绝对路径
kill_svc() {          # $1=名称 $2=匹配串(已含绝对路径锚)
  ...
  if [ -n "$pid" ] && kill -0 "$pid" 2>/dev/null; then
    ...
    # 无 PID 文件(历史裸启)才用 pkill 兜底 — 锚定 ${D2D} 绝对路径, 不误伤其他项目
...
if [ "$ACTION" = "stop" ] || [ "$ACTION" = "restart" ]; then
  ...
  # dsh web 命令行不含仓库路径, 按端口锚定(不误杀其他端口的 dsh 实例)
  pkill -TERM -f "dsh --profile web --port ${WEB_PORT:-8899}" 2>/dev/null || true
