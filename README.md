...
- **资产面** — 测绘四家聚合(FOFA/鹰图/Quake/ZoomEye, 配额排序+凭据检测) + 收集引擎(ICP→测绘反哺→crt.sh→字典爆破→探活指纹, 内置分级字典) + 指纹→策略卡反查注入(严格门)
- **执行面** — cdp-proxy 浏览器代理(登录态/JS 渲染目标, scope 白名单 + 请求级拦截) + 企业代理/自签证书 TLS 兜底(默认严格, 注入 CA)
...
```bash
export D2D_CA_BUNDLE=/path/corporate-ca.pem   # worker curl 统一追加信任(runCurl 自动 --cacert)
export D2D_UPSTREAM_PROXY=proxy.corp:8080     # egress-gateway 经企业代理出网(scope/审计仍在本网关强制)
# dsh 宿主(模型 API)侧: NODE_EXTRA_CA_CERTS=/path/corporate-ca.pem  # 追加信任, 内置 fetch 生效
# macOS 导出系统根: security find-certificate -a -p /Library/Keychains/System.keychain > corp-ca.pem
```

worker 侧 `-k/--insecure/--cacert` 已被重放门拉黑——TLS 放宽只能走上面的宿主配置。诊断：`node scripts/ops/doctor.mjs`（代理可达/证书包可解析/放宽开关污染/NO_PROXY 保护区四项检查）。
